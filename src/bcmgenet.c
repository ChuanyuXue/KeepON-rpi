...
/* KeepON specific
 * Max L2 frame: 1500 (MTU) + (6 + 6 + 2) (src/dst/type) + 4 (FCS) = 1518 bytes
 * Max L1 frame: 1518 + 8 (preamble) + 12 (interframe gap) = 1538 bytes
 * https://community.cisco.com/t5/other-network-architecture-subjects/why-the-mtu-size-is-1500/td-p/105418
*/
#define ETH_FCS_LEN 4
#define TX_BATCH_SIZE batch_size
#define SKB_SIZE (pkt_size + 60) // Default 1518 + 60 = 1578
#define PKT_TRANS_TIME_NS ((pkt_size + 20) * 8)
#define POLL_INTERVAL_MIN_US (max(0, ((PKT_TRANS_TIME_NS * TX_BATCH_SIZE) / 1000) - 100))
#define POLL_INTERVAL_MAX_US (max(0, ((PKT_TRANS_TIME_NS * TX_BATCH_SIZE) / 1000) - 100))
#define PROD_FETCH_SIZE 32
#define INSERT_MIN_THRESHOLD 3
#define DUAL_CLOCK_MAX_ERROR_NS 16
#define TX_THRESHOLD_DEFAULT 128
#define RX_POLL_CORE_INDEX 2
#define TX_POLL_CORE_INDEX 3

#define DEBUG_MAC_BYTE_0 0x01
#define DEBUG_MAC_BYTE_1 0x02
#define DEBUG_MAC_BYTE_2 0x03
#define DEBUG_MAC_BYTE_3 0x04
#define DEBUG_MAC_BYTE_4 0x05
#define DEBUG_MAC_BYTE_5 0x06
#define DEBUG_MAC_PROTOCOL 0x4399

#ifndef SCM_TIMESTAMP
#define SCM_TIMESTAMP 61
#endif

/* KeepON Module Parameters*/
static int batch_size = 8;
MODULE_PARM_DESC(
	batch_size,
	"Number of packets to send in a single register write (<= 512)");

static int pkt_size = 1518;
MODULE_PARM_DESC(pkt_size, "Packet size on wire (<= 1518)");

static unsigned int idle_mask = 0xFFFFFFFF^0x3FFFFFFF;
static unsigned int slot_masks[GENET_MAX_MQ_CNT + 1] = {
	[0] = 0x3FFFFFFF, // slot 0 - 30 are real-time slots
	[1] = 0x00000000, // no allocation
	[2] = 0x00000000, // no allocation
	[3] = 0x00000000, // no allocation
	[4] = 0x00000000, // no allocation
};
MODULE_PARM_DESC(slot_masks, "Bitmask of slot allocation for each queue (default 0x3FFFFFFF,0,0,0,0)");

static int debug_mode = 0; // Default value of 0
MODULE_PARM_DESC(debug_mode, "0: no debug, 1: debug");

static int batch_size_set(const char *val, const struct kernel_param *kp)
{
	int n = 0, ret;
	ret = kstrtoint(val, 10, &n);
	if (ret != 0 || n < 1 || n > 512) // Reasonable limits
		return -EINVAL;
	return param_set_int(val, kp);
}

static int pkt_size_set(const char *val, const struct kernel_param *kp)
{
	int n = 0, ret;
	ret = kstrtoint(val, 10, &n);
	if (ret != 0 || n < 0 || n > 1518) // Reasonable limits
		return -EINVAL;
	return param_set_int(val, kp);
}


static int slot_masks_set(const char *val, const struct kernel_param *kp)
{
	unsigned int masks[GENET_MAX_MQ_CNT + 1] = {0};
	...
	saveptr = str;
	while ((token = strsep(&saveptr, ",")) != NULL && queue < (GENET_MAX_MQ_CNT + 1)) {
		if (*token == '\0')
			continue;
			
		ret = kstrtou32(token, 0, &masks[queue]);
		if (ret != 0) {
			pr_warn("bcmgenet: Invalid slot mask for queue %d: %s\n", queue, token);
			kfree(str);
			return -EINVAL;
		}
		queue++;
	...
	for (int i = 0; i < (GENET_MAX_MQ_CNT + 1); i++) {
		if (total_slots & masks[i]) {
			pr_warn("bcmgenet: Slot conflict detected - queue %d mask 0x%x conflicts with previous allocations 0x%x\n", 
				i, masks[i], total_slots);
			return -EINVAL;
		}
		total_slots |= masks[i];
	}
	...
	/* Update the idle_mask*/
	idle_mask = 0xFFFFFFFF^total_slots;
	return 0;
}
...
static int debug_set(const char *val, const struct kernel_param *kp)
{
	int n = 0, ret;
	ret = kstrtoint(val, 10, &n);
	if (ret != 0 || n < 0 || n > 1) // Reasonable limits
		return -EINVAL;
	return param_set_int(val, kp);
}

static const struct kernel_param_ops batch_size_ops = {
	.set = batch_size_set,
	.get = param_get_int,
};

static const struct kernel_param_ops pkt_size_ops = {
	.set = pkt_size_set,
	.get = param_get_int,
};

static const struct kernel_param_ops slot_masks_ops = {
	.set = slot_masks_set,
	.get = param_get_uint,
};
...
module_param_cb(batch_size, &batch_size_ops,
		&batch_size, 0660);
module_param_cb(pkt_size, &pkt_size_ops, &pkt_size, 0660);
module_param_cb(slot_masks, &slot_masks_ops, &slot_masks, 0660);
module_param_cb(debug_mode, &debug_ops, &debug_mode, 0660);
...
#if ENABLE_DUAL_CLOCK_MODE
		u32 reg = bcmgenet_hfb_reg_readl(priv, HFB_CTRL);
		reg &= ~RBUF_HFB_EN;
		bcmgenet_hfb_reg_writel(priv, reg, HFB_CTRL);
#endif
}
...
static int bcmgenet_get_ts_info(struct net_device *dev,
				struct kernel_ethtool_ts_info *info)
{
	struct bcmgenet_priv *priv = netdev_priv(dev);

	/* Get default software timestamping cap */
...
#if ENABLE_DUAL_CLOCK_MODE
	if (priv->ptp_clock_unified) {
		info->phc_index = ptp_clock_index(priv->ptp_clock_unified);
	} else if (priv->ptp_clock_tx) {
		info->phc_index = ptp_clock_index(priv->ptp_clock_tx);
	} else if (priv->ptp_clock_rx) {
		info->phc_index = ptp_clock_index(priv->ptp_clock_rx);
	} else {
		info->phc_index = -1;
	}
#else
	if (priv->ptp_clock_tx) {
		info->phc_index = ptp_clock_index(priv->ptp_clock_tx);
	} else if (priv->ptp_clock_rx) {
		info->phc_index = ptp_clock_index(priv->ptp_clock_rx);
	} else {
...
static int bcmgenet_hwtstamp_ioctl(struct net_device *dev, struct ifreq *ifr)
{
	struct bcmgenet_priv *priv = netdev_priv(dev);
	...
	// For GET operations, just return the current config
	if (ifr->ifr_ifru.ifru_ivalue == SIOCGHWTSTAMP) {
	...
	case HWTSTAMP_TX_OFF:
		priv->hw_txstamp_en = false;
		break;
	case HWTSTAMP_TX_ON:
		priv->hw_txstamp_en = true;
		break;
	default:
		return -EINVAL;
	}
...
static int bcmgenet_eth_ioctl(struct net_device *dev,
					     struct ifreq *ifr, int cmd)
{
	...
	case SIOCSHWTSTAMP:
	case SIOCGHWTSTAMP:
		return bcmgenet_hwtstamp_ioctl(dev, ifr);
	default:
...
static const struct ethtool_ops bcmgenet_ethtool_ops = {
	...
	.get_ts_info		= bcmgenet_get_ts_info,
...
static __maybe_unused struct enet_cb *bcmgenet_get_txcb(struct bcmgenet_priv *priv,
...
static __maybe_unused struct enet_cb *bcmgenet_put_txcb(struct bcmgenet_priv *priv,
					 struct bcmgenet_tx_ring *ring)
{
	struct enet_cb *tx_cb_ptr;
					  ...
					  struct bcmgenet_tx_ring *ring)
{
	...
	unsigned int old_c_index = ring->c_index;
	unsigned int new_c_index =
		bcmgenet_tdma_ring_readl(priv, ring->index, TDMA_CONS_INDEX) &
		DMA_C_INDEX_MASK;

	if (new_c_index != old_c_index) {
		completed = (new_c_index - old_c_index) & DMA_C_INDEX_MASK;

		for (int i = 0; i < completed; i++) {
			index = (old_c_index + i) % ring->size;
			if (ring->descriptor_state[index] && debug_mode == 0) {
				cb = &ring->cbs[index];
				// Mov padding from critical path to here
				// memset(cb->skb->data, 0x00, SKB_SIZE);
				len_stat = ((SKB_SIZE + ETH_FCS_LEN)
					    << DMA_BUFLENGTH_SHIFT) |
					   (priv->hw_params->qtag_mask
					    << DMA_TX_QTAG_SHIFT) |
					   DMA_SOP | DMA_EOP;
				dmadesc_set(priv, cb->bd_addr,
					    dma_unmap_addr(cb, dma_addr),
					    len_stat);
				ring->descriptor_state[index] = false;
			}
		}

		// Update ring index
		ring->c_index = new_c_index;
		ring->prod_index = (ring->c_index + TX_BATCH_SIZE) &
				   DMA_P_INDEX_MASK;
		bcmgenet_tdma_ring_writel(priv, ring->index, ring->prod_index,
					  TDMA_PROD_INDEX);

		// Update clock
		ring->packet_count += completed;
	}

	return (new_c_index - old_c_index) & DMA_C_INDEX_MASK;
}
...
static int bcmgenet_tx_poll(void *data)
{
	struct bcmgenet_tx_ring *ring = (struct bcmgenet_tx_ring *)data;
	struct net_device *dev = ring->priv->dev;
	...
	while (likely(!kthread_should_stop())) {
		...
		if (POLL_INTERVAL_MIN_US > 0) {
			usleep_range(POLL_INTERVAL_MIN_US, POLL_INTERVAL_MAX_US);
		}
	}
	return 0;
}
...
static int get_earliest_slot(struct bcmgenet_tx_ring *ring, int queue_index)
{
	int index;
	for (int i = 0; i < ring->size; i++) {
		index = (ring->c_index + i + INSERT_MIN_THRESHOLD) % ring->size;
		if ((slot_masks[queue_index] & (1U << index)) && !ring->descriptor_state[index]) {
			return index;
		}
	}
	return -1;
...
static int get_earliest_nrt_slot(struct bcmgenet_tx_ring *ring)
{
	int index;
	for (int i = 0; i < ring->size; i++) {
		index = (ring->c_index + i + INSERT_MIN_THRESHOLD) % ring->size;
		
		if ((idle_mask & (1U << index)) && !ring->descriptor_state[index]) {
			return index;
		}
	}
	return -1;
...
static int bcmgenet_get_tx_descriptor_index(struct sk_buff *skb,
					   struct bcmgenet_tx_ring *ring)
{
	...
	int queue_index;
	int index;
	unsigned int queue_slot_mask;

	queue_index = skb_get_queue_mapping(skb);
	if (queue_index >= GENET_MAX_MQ_CNT + 1) {
		return -1;
	}

	queue_slot_mask = slot_masks[queue_index];

	current_time =
		ring->packet_count * ring->clock_freq + ring->clock_offset;

	if (queue_slot_mask && skb->tstamp != 0) {
		scheduled_time = skb->tstamp;
		index = (scheduled_time - ring->clock_offset) /
			ring->clock_freq % ring->size;

		/* If 1) the incoming packet is late, 2) the slot is not allocated, or 3) the slot is already used, find the earliest slot */
		if (scheduled_time < current_time || !(queue_slot_mask & (1U << index)) || ring->descriptor_state[index]) {
			return get_earliest_slot(ring, queue_index);
		}
		
		/* Drop if the packet is too early
		 * This should never happen because of pre-buffering
		 */
		if (scheduled_time >
		    current_time + ring->size * ring->clock_freq) {
			return -1;
		}

		return index;
	} else {
		return get_earliest_nrt_slot(ring);
	}
}

static void bcmgenet_add_tx_timestamp(struct sk_buff *skb, int index,
					 struct bcmgenet_tx_ring *ring)
{
	struct skb_shared_hwtstamps shhwtstamps;
	uint64_t scheduled_time;

	scheduled_time = (ring->packet_count +
			  (index - ring->c_index + ring->size) % ring->size) *
				 ring->clock_freq +
			 ring->clock_offset;

	if (skb->tstamp != 0 && skb->tstamp != scheduled_time) {
		netdev_err(ring->priv->dev, "[xmit error] skb->tstamp is not equal to scheduled_time\n");
	}

	memset(&shhwtstamps, 0, sizeof(shhwtstamps));
	shhwtstamps.hwtstamp = ns_to_ktime(scheduled_time);
...
static netdev_tx_t bcmgenet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (skb_shinfo(skb)->nr_frags > 1) {
		if (net_ratelimit())
			netdev_err(priv->dev, "[xmit error] pkt has %d frags\n",
	...
	if (skb->len > SKB_SIZE) {
		if (net_ratelimit())
			netdev_err(priv->dev, "[xmit error] pkt length is too long\n");
		dev->stats.tx_dropped++;
		dev->stats.tx_errors++;
		goto out;
	}

	index = bcmgenet_get_tx_descriptor_index(skb, ring);
	if (index == -1) {
		dev->stats.tx_dropped++;
		goto out;
	}
	...
	if (cb->skb) {
		memset(cb->skb->data, 0x00, SKB_SIZE);
		skb_copy_header(cb->skb, skb);
		memcpy(cb->skb->data, skb->data, skb->len);
		dma_sync_single_for_device(&priv->pdev->dev,
					   dma_unmap_addr(cb, dma_addr),
					   SKB_SIZE, DMA_TO_DEVICE);
	}

	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		len_stat = ((SKB_SIZE - ETH_FCS_LEN) << DMA_BUFLENGTH_SHIFT) |
			   (priv->hw_params->qtag_mask << DMA_TX_QTAG_SHIFT) |
			   DMA_SOP | DMA_EOP | DMA_TX_APPEND_CRC;
		len_stat |= DMA_TX_DO_CSUM;
	} else {
		len_stat = (SKB_SIZE << DMA_BUFLENGTH_SHIFT) |
			   (priv->hw_params->qtag_mask << DMA_TX_QTAG_SHIFT) |
			   DMA_SOP | DMA_EOP | DMA_TX_APPEND_CRC;
		len_stat &= ~DMA_TX_DO_CSUM;
	}

	dmadesc_set(priv, cb->bd_addr, dma_unmap_addr(cb, dma_addr), len_stat);

	ring->descriptor_state[index] = true;

	if (do_timestamp) {
		bcmgenet_add_tx_timestamp(skb, index, ring);
	}
				     ...
				     unsigned int budget)
{
	...
	rxpkttoprocess = rxpkttoprocess <= budget ? rxpkttoprocess : budget;
	ring->packet_count += rxpkttoprocess;
	...
	while (rxpktprocessed < rxpkttoprocess) {
		...
		if (priv->hw_rxstamp_en) {
			...
			if (shhwtstamps) {
#if ENABLE_DUAL_CLOCK_MODE
				rx_time = (ring->packet_count -
					   (rxpkttoprocess + discards) +
					   rxpktprocessed) *
						  ring->clock_freq +
					  ring->clock_offset;
#else
				rx_time =
					priv->tx_rings[0].packet_count *
						priv->tx_rings[0]
							.clock_freq +
					priv->tx_rings[0].clock_offset;
#endif

				memset(shhwtstamps, 0, sizeof(*shhwtstamps));
				shhwtstamps->hwtstamp = ns_to_ktime(rx_time);
			} 
		}
...
#if ENABLE_DUAL_CLOCK_MODE
static int bcmgenet_rx_poll_thread(void *data)
{
	struct bcmgenet_rx_ring *ring = (struct bcmgenet_rx_ring *)data;
	struct net_device *dev = ring->priv->dev;
	...
	while (likely(!kthread_should_stop())) {
		spin_lock_bh(&ring->lock);
		bcmgenet_desc_rx(ring, PROD_FETCH_SIZE);
		spin_unlock_bh(&ring->lock);
...
static void bcmgenet_init_tx_desc(struct bcmgenet_priv *priv,
				  struct bcmgenet_tx_ring *ring,
				  unsigned int index)
{
	struct enet_cb *tx_cb_ptr = &ring->cbs[index];
	struct sk_buff *skb;
	dma_addr_t mapping;
	u32 len_stat;

	bool default_pkt = debug_mode &&
			       !(idle_mask & (1U << index));

	// Allocate a new skb
	skb = dev_alloc_skb(SKB_SIZE + ETH_FCS_LEN);
	if (!skb) {
		netdev_err(priv->dev, "Failed to allocate SKB for init desc\n");
		return;
	}

	// Add Ethernet header for debugging
	if (default_pkt) {
		struct ethhdr *eth = (struct ethhdr *)skb_put(skb, ETH_HLEN);
		eth->h_source[0] = DEBUG_MAC_BYTE_0;
		eth->h_source[1] = DEBUG_MAC_BYTE_1;
		eth->h_source[2] = DEBUG_MAC_BYTE_2;
		eth->h_source[3] = DEBUG_MAC_BYTE_3;
		eth->h_source[4] = DEBUG_MAC_BYTE_4;
		eth->h_source[5] = DEBUG_MAC_BYTE_5;
		eth->h_dest[0] = DEBUG_MAC_BYTE_0;
		eth->h_dest[1] = DEBUG_MAC_BYTE_1;
		eth->h_dest[2] = DEBUG_MAC_BYTE_2;
		eth->h_dest[3] = DEBUG_MAC_BYTE_3;
		eth->h_dest[4] = DEBUG_MAC_BYTE_4;
		eth->h_dest[5] = DEBUG_MAC_BYTE_5;
		eth->h_proto = htons(DEBUG_MAC_PROTOCOL);
		memset(skb_put(skb, SKB_SIZE - ETH_HLEN + ETH_FCS_LEN), 0x00,
		       SKB_SIZE - ETH_HLEN + ETH_FCS_LEN);
	} else {
		memset(skb_put(skb, SKB_SIZE + ETH_FCS_LEN), 0x00,
		       SKB_SIZE + ETH_FCS_LEN);
	}
	...
	if (dma_mapping_error(&priv->pdev->dev, mapping)) {
		dev_kfree_skb_any(skb);
		netdev_err(priv->dev, "Failed to map init desc\n");
		return;
	}
	...
	ring->prod_index = (ring->prod_index + 1) & DMA_P_INDEX_MASK;
	ring->free_bds--;
}
				  ...
				  unsigned int start_ptr, unsigned int end_ptr)
{
	...
	if (index == 0) {
		ring->packet_count = 0;
		ring->clock_offset = (ktime_get_real_ns() / 1000000000) * 1000000000;
		ring->clock_freq = PKT_TRANS_TIME_NS;

		netdev_info(priv->dev, "clock freq: %llu\n", ring->clock_freq);
		netdev_info(priv->dev, "clock offset: %llu\n", ring->clock_offset);
	}
	...
	// Allocate active state tracking array
	ring->descriptor_state = kcalloc(size, sizeof(bool), GFP_KERNEL);
	if (!ring->descriptor_state) {
		netdev_err(priv->dev,
			   "failed to allocate tx active state array\n");
		return;
	}

	// Initialize all descriptors in the ring
	if (index == 0) { // Queue 0
		unsigned int i;
		spin_lock(&ring->lock);
		for (i = 0; i < size; i++) {
			bcmgenet_init_tx_desc(priv, ring, i);
		}
		bcmgenet_tdma_ring_writel(priv, index, ring->prod_index,
					  TDMA_PROD_INDEX);
		bcmgenet_tdma_ring_writel(priv, index, TX_THRESHOLD_DEFAULT,
					  DMA_MBUF_DONE_THRESH);
		spin_unlock(&ring->lock);

		// Start the poll thread
		ring->polling_thread = kthread_create_on_cpu(
			bcmgenet_tx_poll, ring, TX_POLL_CORE_INDEX, "bcmgenet_tx_poll");
		if (!IS_ERR(ring->polling_thread)) {
			wake_up_process(ring->polling_thread);
			ring->int_disable(ring);
			netdev_info(priv->dev,
				    "started tx poll thread for queue %d\n",
				    index);
			netdev_info(priv->dev, "slot mask 0: %x\n",
				    slot_masks[0]);
			netdev_info(priv->dev, "slot mask 1: %x\n",
				    slot_masks[1]);
			netdev_info(priv->dev, "slot mask 2: %x\n",
				    slot_masks[2]);
			netdev_info(priv->dev, "slot mask 3: %x\n",
				    slot_masks[3]);
			netdev_info(priv->dev, "slot mask 4: %x\n",
				    slot_masks[4]);
			netdev_info(priv->dev, "idle mask: %x\n", idle_mask);
			netdev_info(priv->dev, "size: %d\n", pkt_size);
			netdev_info(priv->dev, "batch size: %d\n", batch_size);
			netdev_info(priv->dev, "poll interval lb: %d\n",
				    POLL_INTERVAL_MIN_US);
			netdev_info(priv->dev, "poll interval ub: %d\n",
				    POLL_INTERVAL_MAX_US);
		} else {
			netdev_err(priv->dev,
				   "failed to create tx poll thread\n");
			ring->polling_thread = NULL;
		}
	} 
}
				 ...
				 unsigned int start_ptr, unsigned int end_ptr)
{
	...
	if (index == DESC_INDEX) {
		ring->packet_count = 0;
		ring->clock_offset = ktime_get_real_ns();
		ring->clock_freq = PKT_TRANS_TIME_NS;
	}
	...
	if (index == DESC_INDEX) {
		ring->polling_thread = kthread_create_on_cpu(
			bcmgenet_rx_poll_thread, ring, RX_POLL_CORE_INDEX, "bcmgenet_rx_poll");
		if (!IS_ERR(ring->polling_thread)) {
			wake_up_process(ring->polling_thread);
			ring->int_disable(ring);
			netdev_info(priv->dev,
				    "started rx poll thread for queue %d\n",
				    index);
		} else {
			netdev_err(priv->dev,
				   "failed to create rx poll thread\n");
			ring->polling_thread = NULL;
		}
	} else {
		netif_napi_add(priv->dev, &ring->napi, bcmgenet_rx_poll);
...
static __maybe_unused void bcmgenet_enable_tx_napi(struct bcmgenet_priv *priv)
{
	unsigned int i;
	struct bcmgenet_tx_ring *ring;
...
static __maybe_unused void bcmgenet_disable_tx_napi(struct bcmgenet_priv *priv)
{
	unsigned int i;
	struct bcmgenet_tx_ring *ring;
...
static __maybe_unused void bcmgenet_fini_tx_napi(struct bcmgenet_priv *priv)
{
	unsigned int i;
	struct bcmgenet_tx_ring *ring;
...
static void bcmgenet_fini_dma(struct bcmgenet_priv *priv)
{
	...
	for (i = 0; i < priv->hw_params->tx_queues; i++) {
		...
		kfree(priv->tx_rings[i].descriptor_state);

		if (i == 0 && priv->tx_rings[i].polling_thread) {
			kthread_stop(priv->tx_rings[i].polling_thread);
			priv->tx_rings[i].polling_thread = NULL;
		}
	}

#if ENABLE_DUAL_CLOCK_MODE
	if (priv->rx_rings[DESC_INDEX].polling_thread) {
		kthread_stop(priv->rx_rings[DESC_INDEX].polling_thread);
		priv->rx_rings[DESC_INDEX].polling_thread = NULL;
	}
#endif
...
static const struct net_device_ops bcmgenet_netdev_ops = {
	...
	.ndo_eth_ioctl		= bcmgenet_eth_ioctl,
...
static int bcmgenet_probe(struct platform_device *pdev)
{
	...
	if (priv->ptp_clock_tx) {
		ptp_clock_unregister(priv->ptp_clock_tx);
		priv->ptp_clock_tx = NULL;
	}
	if (priv->ptp_clock_rx) {
		ptp_clock_unregister(priv->ptp_clock_rx);
		priv->ptp_clock_rx = NULL;
	}
	if (priv->ptp_clock_unified) {
		ptp_clock_unregister(priv->ptp_clock_unified);
		priv->ptp_clock_unified = NULL;
	}

	netdev_info(priv->dev, "bcmgenet: calling PTP init\n");
	bcmgenet_ptp_init(priv);
...
BCMGENET_PTP_WRAP_FUNCS(tx, TX)
BCMGENET_PTP_WRAP_FUNCS(rx, RX)
#if ENABLE_DUAL_CLOCK_MODE
BCMGENET_PTP_WRAP_FUNCS(combined, COMBINED)
#endif

static struct bcmgenet_priv *bcmgenet_clock_to_priv(struct ptp_clock_info *ptp,
						    enum bcmgenet_clk_type type)
{
	switch (type) {
	case BCMGENET_TX_CLK:
		return container_of(ptp, struct bcmgenet_priv,
				    ptp_clock_info_tx);
	case BCMGENET_RX_CLK:
		return container_of(ptp, struct bcmgenet_priv,
				    ptp_clock_info_rx);
#if ENABLE_DUAL_CLOCK_MODE
	case BCMGENET_COMBINED_CLK:
		return container_of(ptp, struct bcmgenet_priv,
				    ptp_clock_info_unified);
#endif
	default:
		/* Should never happen */
		WARN_ON(1);
...
#if ENABLE_DUAL_CLOCK_MODE
static uint64_t bcmgenet_get_combined_time(struct bcmgenet_priv *priv)
{
	uint64_t tx_time, rx_time;
	uint64_t combined_time;

	tx_time = priv->tx_rings[0].packet_count * priv->tx_rings[0].clock_freq +
		  priv->tx_rings[0].clock_offset;
	rx_time = priv->rx_rings[DESC_INDEX].packet_count *
			  priv->rx_rings[DESC_INDEX].clock_freq +
		  priv->rx_rings[DESC_INDEX].clock_offset;

	if (tx_time > rx_time && tx_time - rx_time > PKT_TRANS_TIME_NS) {
		priv->rx_rings[DESC_INDEX].clock_offset += (tx_time - rx_time);
		combined_time = tx_time;
	} else if (rx_time > tx_time && rx_time - tx_time > PKT_TRANS_TIME_NS) {
		priv->tx_rings[0].clock_offset += (rx_time - tx_time);
		combined_time = rx_time;
	} else {
		combined_time =
			(max(tx_time, rx_time) +
			 min(tx_time + PKT_TRANS_TIME_NS, rx_time + PKT_TRANS_TIME_NS)) /
			2;
	}
...
int bcmgenet_ptp_gettime64(struct ptp_clock_info *ptp, struct timespec64 *ts,
			   enum bcmgenet_clk_type type)
{
	struct bcmgenet_priv *priv = bcmgenet_clock_to_priv(ptp, type);
	uint64_t ns;
	unsigned long flags_tx, flags_rx;

	switch (type) {
	case BCMGENET_TX_CLK:
		spin_lock_irqsave(&priv->tx_rings[0].lock, flags_tx);
		ns = priv->tx_rings[0].packet_count *
			     priv->tx_rings[0].clock_freq +
		     priv->tx_rings[0].clock_offset;
		spin_unlock_irqrestore(&priv->tx_rings[0].lock, flags_tx);
		break;
	case BCMGENET_RX_CLK:
		spin_lock_irqsave(&priv->rx_rings[DESC_INDEX].lock, flags_rx);
		ns = priv->rx_rings[DESC_INDEX].packet_count *
			     priv->rx_rings[DESC_INDEX].clock_freq +
		     priv->rx_rings[DESC_INDEX].clock_offset;
		spin_unlock_irqrestore(&priv->rx_rings[DESC_INDEX].lock,
				       flags_rx);
		break;
#if ENABLE_DUAL_CLOCK_MODE
	case BCMGENET_COMBINED_CLK:
		spin_lock_irqsave(&priv->tx_rings[0].lock, flags_tx);
		spin_lock_irqsave(&priv->rx_rings[DESC_INDEX].lock, flags_rx);
		ns = bcmgenet_get_combined_time(priv);
		spin_unlock_irqrestore(&priv->rx_rings[DESC_INDEX].lock,
				       flags_rx);
		spin_unlock_irqrestore(&priv->tx_rings[0].lock, flags_tx);
		break;
...
int bcmgenet_ptp_settime64(struct ptp_clock_info *ptp,
...
#if ENABLE_DUAL_CLOCK_MODE
	case BCMGENET_COMBINED_CLK:
		// bcmgenet_ptp_settime64(ptp, ts, BCMGENET_TX_CLK);
		// bcmgenet_ptp_settime64(ptp, ts, BCMGENET_RX_CLK);
		spin_lock_irqsave(&priv->tx_rings[0].lock, flags_tx);
		spin_lock_irqsave(&priv->rx_rings[DESC_INDEX].lock, flags_rx);
		priv->tx_rings[0].clock_offset =
			ns - priv->tx_rings[0].packet_count *
...
int bcmgenet_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta,
			 enum bcmgenet_clk_type type)
{
	struct bcmgenet_priv *priv = bcmgenet_clock_to_priv(ptp, type);
	unsigned long flags_tx, flags_rx;
	...
	case BCMGENET_TX_CLK:
		spin_lock_irqsave(&priv->tx_rings[0].lock, flags_tx);
		priv->tx_rings[0].clock_offset += delta;
		spin_unlock_irqrestore(&priv->tx_rings[0].lock, flags_tx);
		break;
	case BCMGENET_RX_CLK:
		spin_lock_irqsave(&priv->rx_rings[DESC_INDEX].lock, flags_rx);
		priv->rx_rings[DESC_INDEX].clock_offset += delta;
		spin_unlock_irqrestore(&priv->rx_rings[DESC_INDEX].lock,
				       flags_rx);
		break;
#if ENABLE_DUAL_CLOCK_MODE
...
int bcmgenet_ptp_adjfine(struct ptp_clock_info *ptp, long scaled_ppm,
			 enum bcmgenet_clk_type type)
{
	struct bcmgenet_priv *priv = bcmgenet_clock_to_priv(ptp, type);
	unsigned long flags_tx, flags_rx;
	...
	case BCMGENET_TX_CLK:
		...
		time_cycle = priv->tx_rings[0].packet_count;
		adj_freq = div_s64(PKT_TRANS_TIME_NS * scaled_ppm, 65536LL);
		adj_freq = div_s64(adj_freq, 1000000LL);

		time_freq = PKT_TRANS_TIME_NS + adj_freq;
		priv->tx_rings[0].clock_offset -=
			(time_freq - priv->tx_rings[0].clock_freq) * time_cycle;
		priv->tx_rings[0].clock_freq = time_freq;
	...
	case BCMGENET_RX_CLK:
		...
		time_cycle = priv->rx_rings[DESC_INDEX].packet_count;
		adj_freq = div_s64(PKT_TRANS_TIME_NS * scaled_ppm, 65536LL);
		adj_freq = div_s64(adj_freq, 1000000LL);

		time_freq = PKT_TRANS_TIME_NS + adj_freq;
		priv->rx_rings[DESC_INDEX].clock_offset -=
			(time_freq - priv->rx_rings[DESC_INDEX].clock_freq) *
			time_cycle;
		priv->rx_rings[DESC_INDEX].clock_freq = time_freq;
...
static struct ptp_clock *bcmgenet_ptp_clock_init(struct bcmgenet_priv *priv,
						 struct ptp_clock_info *info,
						 const char *name,
						 void *gettime, void *settime,
						 void *adjtime, void *adjfine)
{
	struct ptp_clock *clock;

	info->owner = THIS_MODULE;
	snprintf(info->name, PTP_CLOCK_NAME_LEN, "genet ptp %s", name);
	info->max_adj = 1000000000;
	info->n_alarm = 0;
	info->n_ext_ts = 0;
	info->n_per_out = 0;
	info->pps = 0;
	info->gettime64 = gettime;
	info->settime64 = settime;
	info->adjtime = adjtime;
	info->adjfine = adjfine;

	clock = ptp_clock_register(info, &priv->dev->dev);
	if (IS_ERR(clock)) {
		pr_err("bcmgenet: failed to register %s PTP clock: %ld\n", name,
		       PTR_ERR(clock));
		netdev_err(priv->dev, "%s ptp clock registration failed\n",
			   name);
		return NULL;
	}
...
void bcmgenet_ptp_init(struct bcmgenet_priv *priv)
{
	netdev_info(priv->dev, "bcmgenet: initializing PTP clock\n");

	priv->ptp_clock_tx = bcmgenet_ptp_clock_init(
		priv, &priv->ptp_clock_info_tx, "tx", bcmgenet_ptp_tx_gettime64,
		bcmgenet_ptp_tx_settime64, bcmgenet_ptp_tx_adjtime,
		bcmgenet_ptp_tx_adjfine);

	priv->ptp_clock_rx = bcmgenet_ptp_clock_init(
		priv, &priv->ptp_clock_info_rx, "rx", bcmgenet_ptp_rx_gettime64,
		bcmgenet_ptp_rx_settime64, bcmgenet_ptp_rx_adjtime,
		bcmgenet_ptp_rx_adjfine);

#if ENABLE_DUAL_CLOCK_MODE
	priv->ptp_clock_unified = bcmgenet_ptp_clock_init(
		priv, &priv->ptp_clock_info_unified, "combined",
		bcmgenet_ptp_combined_gettime64,
		bcmgenet_ptp_combined_settime64,
		bcmgenet_ptp_combined_adjtime,
		bcmgenet_ptp_combined_adjfine);
#endif
}
