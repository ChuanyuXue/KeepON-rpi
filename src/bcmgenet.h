...
struct bcmgenet_priv {
	...
	/* PTP clock */
	struct ptp_clock *ptp_clock_tx;
	struct ptp_clock_info ptp_clock_info_tx;

	struct ptp_clock *ptp_clock_rx;
	struct ptp_clock_info ptp_clock_info_rx;

	struct ptp_clock *ptp_clock_unified;
	struct ptp_clock_info ptp_clock_info_unified;
};
...
enum bcmgenet_clk_type {
	BCMGENET_TX_CLK,
	BCMGENET_RX_CLK,
	BCMGENET_COMBINED_CLK
};

int bcmgenet_ptp_gettime64(struct ptp_clock_info *ptp, struct timespec64 *ts,
			   enum bcmgenet_clk_type clk_type);
...
#define BCMGENET_PTP_WRAP_FUNCS(type, TYPE)                                    \
	static int bcmgenet_ptp_##type##_gettime64(struct ptp_clock_info *ptp, \
						   struct timespec64 *ts)      \
	{                                                                      \
		return bcmgenet_ptp_gettime64(ptp, ts, BCMGENET_##TYPE##_CLK); \
	}                                                                      \
	static int bcmgenet_ptp_##type##_settime64(                            \
		struct ptp_clock_info *ptp, const struct timespec64 *ts)       \
	{                                                                      \
		return bcmgenet_ptp_settime64(ptp, ts, BCMGENET_##TYPE##_CLK); \
	}                                                                      \
	static int bcmgenet_ptp_##type##_adjtime(struct ptp_clock_info *ptp,   \
						 s64 delta)                    \
	{                                                                      \
		return bcmgenet_ptp_adjtime(ptp, delta,                        \
					    BCMGENET_##TYPE##_CLK);            \
	}                                                                      \
	static int bcmgenet_ptp_##type##_adjfine(struct ptp_clock_info *ptp,   \
						 long scaled_ppm)              \
	{                                                                      \
		return bcmgenet_ptp_adjfine(ptp, scaled_ppm,                   \
					    BCMGENET_##TYPE##_CLK);            \
	}

#define bcmgenet_ptp_SUPPORT
