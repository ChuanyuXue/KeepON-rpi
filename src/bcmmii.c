...
static void bcmgenet_mac_config(struct net_device *dev)
{
	...
	if (phydev->duplex != DUPLEX_FULL) {
		cmd_bits |= CMD_HD_EN | CMD_RX_PAUSE_IGNORE |
			    CMD_TX_PAUSE_IGNORE;
	} else {
	...
	bcmgenet_eee_enable_set(dev, priv->eee.eee_enabled && active,
...
static int bcmgenet_mii_pd_init(struct bcmgenet_priv *priv)
{
	...
	snprintf(mdio_bus_id, MII_BUS_ID_SIZE, "%s-%d", UNIMAC_MDIO_DRV_NAME,
		 priv->pdev->id);

	if (pd->phy_interface != PHY_INTERFACE_MODE_MOCA && pd->mdio_enabled) {
		snprintf(phy_name, MII_BUS_ID_SIZE, PHY_ID_FMT, mdio_bus_id,
			 pd->phy_address);
